...
from moco.moco_dataset_v2 import ListFileDataSet
from moco.mocov3_model import MoCoV3
from transforms import *

from moco.moco_config_v2_pretrain import parser
...
def main_worker(local_rank):
    ...
    if args.evaluate:
        ...
        validate(val_loader, model, -1, "val", local_rank)
        validate(test_loader, model, -1, "test", local_rank)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_loss, train_acc, moco_m = train(train_loader,
                                              model,
                                              optimizer,
                                              epoch,
                                              local_rank)
        ...
        val_loss, val_acc = validate(val_loader, model, epoch, "val", local_rank, moco_m)
        test_loss, test_acc = validate(test_loader, model, epoch, "test", local_rank, moco_m)
...
def train(train_loader, model, optimizer, epoch, local_rank, moco_m=0.00):
    ...
    for step, data in enumerate(train_loader):
        ...
        adjust_learning_rate(optimizer, epoch + step / iters_per_epoch, args)
        ...
        loss = model(vision_q=images[0], vision_k=images[1], m=moco_m)
        ...
        if step % args.print_freq == 0:
            logger.info('Epoch: [{epoch}][{step}/{len}]\t'
                        ...
                        'Acc {acces.val:.3f} ({acces.avg:.3f})'.format(epoch=epoch, step=step, len=len(train_loader),
                                                                       ...
                                                                       losses=losses,
                                                                       acces=acces))
    return losses.avg, acces.avg, moco_m


def validate(val_loader, model, epoch, data_type, local_rank, moco_m=0.99):
    batch_time = AverageMeter()
    losses = AverageMeter()
    acces = AverageMeter()
