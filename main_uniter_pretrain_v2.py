...
from uniter.uniter_dataset_v2 import ListFileDataSet, data_collate

from transforms import *
from transformers import BertTokenizer

from uniter.config_uniter_v2 import parser
from uniter.uniter_model_v2 import Uniter
...
def main_worker(local_rank):
    ...
    if args.evaluate:
        ...
        validate(val_loader, model, (criterion_ce, criterion_kl), -1, "val", label_dict, local_rank, args.task.split(","))
        validate(test_loader, model, (criterion_ce, criterion_kl), -1, "test", label_dict, local_rank, args.task.split(","))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        person_val_acc, style_val_acc, topic_val_acc = validate(val_loader, model, (criterion_ce, criterion_kl), epoch, "val", label_dict, local_rank, args.task.split(","))
        test_loss, expression_test_acc, material_test_acc, \
        person_test_acc, style_test_acc, topic_test_acc = validate(test_loader, model, (criterion_ce, criterion_kl), epoch, "test", label_dict, local_rank, args.task.split(","))
...
def validate(val_loader, model, criterions, epoch, data_type, label_dict, local_rank, task):
    batch_time = AverageMeter()
    losses = AverageMeter()
    expression_acces = AverageMeter()
    material_acces = AverageMeter()
    ...
    with torch.no_grad():
        for step, (vid, data) in enumerate(val_loader):
            ...
            if 'cls' in task:
                ...
                for dim in dims:
                    acc = torch.round(torch.tensor(100.))
                    eval(f"{dim}_acces").update(acc, batch_size)
