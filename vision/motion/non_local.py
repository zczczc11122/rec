...
class NL3DWrapper(nn.Module):
    def __init__(self, block, n_segment, base_model):
...
def make_non_local(net, n_segment, base_model):
    assert base_model == 'resnet18', "non_local 目前只支持 base_model为resnet18"
    import torchvision
    if isinstance(net, torchvision.models.ResNet):
        # 因为resnet18的layer2和layer3 长度只有2，所以layer2[2]或索引更大会报错，如果想支持其他resnet按照它的那个顺序写就ok了
        net.layer2 = nn.Sequential(
            NL3DWrapper(net.layer2[0], n_segment, base_model),
            net.layer2[1]
            # NL3DWrapper(net.layer2[2], n_segment, base_model),
            # net.layer2[3],
        )
        net.layer3 = nn.Sequential(
            NL3DWrapper(net.layer3[0], n_segment, base_model),
            net.layer3[1]#,
            # NL3DWrapper(net.layer3[2], n_segment, base_model),
            # net.layer3[3],
            # NL3DWrapper(net.layer3[4], n_segment, base_model),
            # net.layer3[5],
        )
    else:
